...
""" Find the gradient corresponding to the Phi update using the deformation calculated by comparing Spher to Spher0. An alternate method specifies the current configuration by a Nodes matrix rather than Spher """
function GradientPsi(psivals::Vector{Float64}, Spher::Spheroid, Spher0::Spheroid)
    @assert Spher.Triangles == Spher0.Triangles
    return GradientPsi(psivals, Spher.Nodes, Spher0)
end
...
function gradEnergyPsi!(du, u, p, t)
    xdiff = ForwardDiff.gradient(x::Vector->EnergyFunctional(x[1:3noNodes]::Vector, Spher0.Triangles, u[3noNodes+1:end]), u[1:3noNodes])
    Ψdiff = GradientPsi(u[3noNodes+1:end], u[1:3noNodes], Spher0)
    du[1:3noNodes] = xdiff
...
function show(io::IO, Sim::ODESimulation)
    println(io, "A simulation generated by solving an ODE with Ψ, with $(size(Sim.Sphervec[1].Nodes)[1]) nodes and $(length(Sim.tvec)) timesteps")
end

PlotRadius(Sim::ODESimulation, j::Int; name="Distance from centre: t = $(Sim.tvec[j])") = PlotRadius(Sim.Sphervec[j]; name=name)
ShowPsi(Sim::ODESimulation, j::Int=length(Sim); args...) = ShowPsi(Sim.Sphervec[j]; args...)

"""
    Run the main gradient descent method through DifferentialEquations and save the result as a simulation
"""
function mainODE(Spher::Spheroid, T::Float64; Spher0::Spheroid=Spher)
    t0 = time()
    tspan = (0., T)
    x0vec = [Nodes2xvec(Spher.Nodes); Spher.Psivals]
    # f(u, p, t) = -gradEnergyPsi(u, Spher0)
    ...
    return ODESimulation([Spheroid(xvec2Nodes(sol.u[j][1:3noNodes]), Spher.Triangles, sol.u[j][3noNodes+1:end]) for j in 1:length(sol)], sol.t, time()-t0);
end

"""
    Continue a given simulation, this sometimes causes instabilities and I don't know why
"""
function ContinueSimulation(Sim::ODESimulation, T::Float64; Spher0 = Sim.Sphervec[1])
    Sim2 = mainODE(Sim.Sphervec[end], T)
    return ODESimulation([Sim.Sphervec; Sim2.Sphervec], [Sim.tvec; Sim2.tvec .+ Sim.tvec[end]], Sim.RunTime+Sim2.RunTime)
end

function CheckEnergy(Sim::ODESimulation, j::Int; Loud::Bool=true)
    return EnergyFunctional(Sim.Sphervec[j].Nodes, Sim.Sphervec[j].Triangles, Sim.Sphervec[j].Psivals, Loud=Loud)
end
...
CompareCrossSections(Sim::ODESimulation, z::Number=0., tvals=1:10:101; args...) = CompareCrossSections([Sim.Sphervec[t] for t in tvals], z; names=["t = $t" for t in tvals], args...)